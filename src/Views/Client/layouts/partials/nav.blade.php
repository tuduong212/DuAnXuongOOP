    ...
    <div class="container">
        <nav class="navbar navbar-expand-lg navbar-white">
                        ...
                        @php
                            if (isset($_SESSION['user'])) {
                                if ($_SESSION['user']['type']=='admin') {
                                    echo '<a class="nav-link" href="' . url('admin/') . '">Admin</a>';
                                }
                            }
                        @endphp
                    </li>
            ...
            <div class="order-2 order-lg-3 d-flex align-items-center">
                @if (!isset($_SESSION['user']))
                    <a class="btn btn-primary" class="p-3 border-0 bg-transparent" id="select-language" href="{{ url('login') }}">Login</a>
                @endif

                @if (isset($_SESSION['user']))
                    <a class="btn btn-primary" class="p-3 border-0 bg-transparent" id="select-language" href="{{ url('logout') }}">Logout</a>
                @endif

                <!-- search -->
                <form class="search-bar ">
                    <input class="" id="search-query" name="s" type="search"
